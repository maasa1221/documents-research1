...
カーネル関数は２つの引数をとり、それぞれの類似度を算出する。

おいおいにして、カーネルの引数は再生核ヒルベルト空間じょうに存在する関数の点であり、これらをグラム行列のかたちでまんべんなく計算したとき、グラム行列の対角成分はかならず自分同士になり類似度の基準となることがわかる。このことから対角行列はおいおいにして半正定性をもつので正定値カーネルであるといえる。



再生核ヒルベルト空間であって欲しい理由はユークリッド空間のような有限くうかんではなく無限をもちいたいためがひとつ。このままではヒルベルト空間でよくねとなるがここで登場するものがリプレゼンター定理である。
リプレゼンター定理を有効化するためにヒルベルト空間にわざわざ再生性なんてものをつけていると言っても過言ではない。


？？？？？？


カーネル法はヒルベルト空間でしかつかえない？
だからこそ、有限次元化できるリプレゼンターが必要？？
