...
ナブラがなんであるかと存在意義からかんがえると、この文字のやっていることは偏微分モンスターであることがわかる。
つまりは xyz 方向のすべての方向に対して関数の微分を行ったと３次元方向ベクトルができ、こっれは現実世界における勾配と等しいと考えられる。よって、このナブラは物理学の方面で度々出現する。

### 発散とナブラ

ちな発散の英語はダイバージェンス(divergence)。
なのでその頭文字を取って div と書く
...
ポアソン分布は二項分布の極限である。=> ポアソンの極限定理

## 射影変換とアフィン変換

こいつらは総じて同次変換と呼ばれる。
よくみる(x,y,1)を入力として与えて,3\*3 次元の行列によって写像する変換方法である。これにより平行移動が可能になり、アフィン変換と射影変換を行うことができる。ちなみにアフィン変換は射影変換の一部のケースのことをいう、というのも対となる辺同士が平行になるケース(変換)をアフィン変換と呼ぶ。つまりは射影変換は対となる辺が平行にならない場合の変換もできる。この様が他の壁に射影される様子に似ているため射影変換と呼ばれる。
...
ここではじめてでてくるヒストグラムを用いる系のアルゴリズム。めちゃくちゃ単純。

![](2020-02-17-14-33-25.png)

![](2020-02-17-14-33-46.png)

http://www.thothchildren.com/chapter/5bc358ec51d9305189029edb

![](2020-02-17-14-56-07.png)

![](2020-02-17-14-57-44.png)

![](2020-02-17-14-57-58.png)

![](2020-02-17-15-53-59.png)

![](2020-02-17-16-01-10.png)


クラス内分散とクラス間分散をもとめてその比をもとめる。

まあようするにクラス間分散をおおきくしてクラス内分散をちいさくなるようにしたい。

これの示していることは、クラス同士ははなれていて、それぞれのクラスは集まっていてほしい。そっちのほうが　明確にわかれているということが視覚的にもわかりやすいし理解しやすい。


## フィルタ野郎ども（Prewitt, Sobel, Gaussian, Laplacian)を添えて

こいつらのやりたいことは、畳込みに似ている。
フィルタ内のどの部分を採用するかによってノイズを除去できるかもしれないという考え方から生まれている。
この作業のことをスムージングもしくは平滑化と呼び
、ピクセル単位のノイズとの戦いになりがちな画像処理分野における必須テクの一つ。

平均化フィルタ：すべてのピクセルに等価の重みを与える。

微分フィルタ：ある進行方向に対して差を求める。ただそれだけ(prewitt)。このうち中央により重みをつよめることで(sobelになる)

ガウシアンフィルタ：読んで字のごとくフィルタの重みを正規分布（ガウシアン分布）形状にしたもの、中央にいくにつれ重みが大きくなる。

ラプラシアンフィルタ：読んで字のごとくラプラシアンを用いたフィルタ。ラプラシアンとは勾配のことであり、二次微分を指す言葉なのでその名の通り先程登場した微分フィルタを二重に重ねがけする。ただそれだけ。


もうちょっとわかりやすくほそくすると、まず下図のような一次微分フィルタというものをかんがえる。

これは、行列であらわすことができ、二階のテンソル形状をしていることがわかる。べつにフィルタの形状は二階テンソル以上でなければならない理由はなく一階テンソルすなわちはベクトルの形状でも別に構わない。よって中段のみを抽出してベクトルフィルタとして扱ってもなんら問題はない。

![](2020-02-18-19-37-22.png)

prewittやsobelはこのベクトルフィルタをもとにしてそれを３段に複製していると考えるとわかりやすい。（下図）。
![](2020-02-18-19-37-51.png)
![](2020-02-18-19-37-42.png)
![](2020-02-18-19-37-59.png)
![](2020-02-18-19-38-06.png)
![](2020-02-18-19-38-13.png)

ラプラシアンフィルタもこの一次微分フィルタを基準に考えるとわかりやすい。

ラプラシアン＝勾配＝二次微分なので、この一次微分フィルタの微分を取ったら二次微分になるやんけという単純なかんがえである。この考えから一次微分フィルタを引き合い、縦方向と横方向でそれぞれ求めた二次微分フィルタを合体させれば完成である。

## LoGとDoG

略さずに言うとLaplacian of GaussianとDifference of Gaussianになる。

その言葉の通り、ガウシアンフィルタの二次微分とガウシアンフィルタの差分を求める。
この２つが何をやっているのかを紐解くことにより、この話は特徴点抽出の話に発展してゆく。
特徴点や境界線は周囲と明らかに異なり、色が変化する。
この色の変化が最大になる点を知りたいとなったとき、一次微分を使うことで求めることができるのではないかと考えられる。一次微分でピクセルの変化を知ることができ、二次微分でピクセル変化の変化を知ることができる。
ピクセル変化の変化量が最大になる点を可視化することができる。
これにより、求めたかったピクセル変化の最大点を見つけることができた。これの毎処理としてガウシアンフィルターによる平滑化を加えたものがLoGである。

対して、DoGはLoGの単純かと言えるものである。（近似とも言う）
